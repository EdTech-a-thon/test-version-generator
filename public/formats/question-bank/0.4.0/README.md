# Question Bank Record 0.4.0 fixtures

- [`schema.json`](schema.json) is a version-pinned copy of the public schema whose
  stable identifier is `https://testparrot.com/formats/question-bank/0.4.0/schema.json`.
- [`examples/`](examples/) contains the eight canonical conforming records.
- [`invalid/`](invalid/) contains one-purpose counterexamples and an expected
  application error-code manifest.

The prose contract is in
[`docs/question-bank-record-0.4.0.md`](../../../../docs/question-bank-record-0.4.0.md).
All conforming records can be inspected through Test Parrot's public
record-import boundary.

[`../0.3.0/`](../0.3.0/), [`../0.2.0/`](../0.2.0/) and [`../0.1.0/`](../0.1.0/) are the superseded
versions Test Parrot still reads.
