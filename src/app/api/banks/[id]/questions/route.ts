...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const bank = await db.bank.findFirst({
      ...
      include: { questions: { where: { question: { status: "ACTIVE" } }, include: { question: { include: { tags: true } } }, orderBy: { question: { updatedAt: "desc" } } } },
    });
    if (!bank) return NextResponse.json({ error: "Question bank not found." }, { status: 404 });
    return NextResponse.json({ id: bank.id, name: bank.name, description: bank.description, questions: bank.questions.map(({ question }) => ({ id: question.id, stem: question.stem, difficulty: question.difficulty, tags: question.tags.map((tag) => tag.name) })) });
  } catch (error) {
