...
type Question = { id: string; stem: string; difficulty: number; tags: string[]; stemImageKey?: string | null };
...
export function BankQuestions({ bankId }: { bankId: string }) {
  ...
  return bank?.questions.length ? <>
    ...
    <section className="bank-list">{bank.questions.map((question) => <article className="card" key={question.id}><p className="eyebrow">Difficulty {question.difficulty} of 5</p><h2>{renderMarkup(question.stem)}</h2><QuestionImage imageKey={question.stemImageKey} /><p>{question.tags.length ? question.tags.join(", ") : "No tags yet"}</p><div className="completion-actions"><Link className="secondary" href={`/questions/new?id=${question.id}&bank=${bankId}`}>Edit</Link><button className="secondary" onClick={() => void remove(question.id)}>Remove from bank</button></div></article>)}</section>
