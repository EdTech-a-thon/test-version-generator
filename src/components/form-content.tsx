...
function BubbledItem({ item, number, answerKey }: { item: PrintableItem; number: number; answerKey: boolean }) {
  ...
  return <section className="print-question"><h3>{number}. {renderMarkup(data.stem)}</h3><QuestionImage imageKey={data.stemImageKey} /><ol>{order.map((optionIndex, index) => <li className={answerKey && correctLetters.includes(letter(index)) ? "correct-choice" : ""} key={`${item.position}-${optionIndex}`}><span className="bubble">{letter(index)}</span>{renderMarkup(data.options[optionIndex]?.text)}{data.options[optionIndex]?.imageKey && <QuestionImage imageKey={data.options[optionIndex].imageKey} />}{answerKey && correctLetters.includes(letter(index)) ? <strong> Correct</strong> : null}</li>)}</ol>{answerKey ? <p>Answer: {correctLetters.join(", ")}{data.generatedVariant ? ` (${data.generatedVariant.correct})` : ""}</p> : null}</section>;
}

function FreeResponseItem({ item, number, answerKey }: { item: PrintableItem; number: number; answerKey: boolean }) {
  const data = snapshot(item);
  return <section className="print-question free-response"><h3>FR{number}. {renderMarkup(data.stem)} ({item.points} points)</h3><QuestionImage imageKey={data.stemImageKey} />{answerKey ? <><p><strong>Expected answer:</strong> {data.correctAnswer.join(", ") || "See solution"}</p>{data.solution && <p><strong>Solution:</strong> {data.solution}</p>}{data.rubric && <p><strong>Rubric:</strong> {data.rubric}</p>}</> : <div className="response-space" />}</section>;
}
