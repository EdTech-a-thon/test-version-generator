...
- Underscores are the blanks. The default is written as text too, so the line on screen, the field it turns into and every output are the same characters; the default's blanks are sized to fit beside the ID at the sheet's type. Only departures from the default are stored, as for section wording (ADR-0025).
