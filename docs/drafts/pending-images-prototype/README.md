...
Throwaway scripts from designing ADR-0024. They are evidence and a starting point, not production code. Run them with `bun` from the repository root.
...
`truth.json` and `truth-packet.json` record, for the two sample tests used in ADR-0024, which ripped image (`p<page>#<n>`, in paint order) belongs in each Question's stem or answer choice. A `"passage": true` slot is a passage stored as an image, which an assistant may transcribe or keep as a picture.
