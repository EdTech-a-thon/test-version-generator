...
//      Playwright Chromium.
//   3. Downloads the real DOCX through the application's own Export menu.
...
import { buildExportDocument, planExport, STUDENT_TEST } from '../src/export-plan'
import {
  compareFingerprints,
  describeDifferences,
...
for (const fixture of fixtures) {
  test(`print and DOCX describe the same document: ${fixture.name}`, async ({
    ...
    // The plan the application will build for itself, built here too so the
    // structural comparison has a reference that never went near a renderer.
    ...
    record(
      directory,
      'layout-plan.json',
      JSON.stringify(layoutPlanFingerprint(plan), null, 2),
    )
    ...
    //    actually prints: Export → Print… → Print selected, with the panel's
    //    default selection, which is the student test alone.
    ...
    // 2. The real DOCX, downloaded the way a teacher downloads it.
    ...
    const printed = printDocumentFingerprint(printMarkup, {
      ...
      version: fixture.version.letter,
