...
test('a downloaded account backup restores every Question Bank and image', async ({ page }) => {
  ...
  await expect(panel(page)).toContainText('Your work stays in this browser')
...
test('a file that is not a backup is refused without changing anything', async ({ page }) => {
  ...
  await expect(panel(page).getByRole('alert')).toHaveText('This file is not a Test Parrot account backup.')
...
test('two devices sync through a Test Parrot folder in Google Drive', async ({ browser, baseURL }) => {
  ...
  await openPanel(a)
  await panel(a).getByRole('button', { name: 'Sync across devices with Google Drive' }).click()

  const dialog = a.getByRole('dialog', { name: 'Sync with Google Drive' })
  ...
  // Back from Google, the dialog reopens with the first check earned.
  ...
  await openPanel(b)
  await panel(b).getByRole('button', { name: 'Sync across devices with Google Drive' }).click()
  const dialogB = b.getByRole('dialog', { name: 'Sync with Google Drive' })
  await dialogB.getByRole('button', { name: 'Turn on sync' }).click()
  ...
  await a.goto('/question-banks')
  await openPanel(a)
  await panel(a).getByRole('button', { name: 'Sync settings' }).click()
  await a.getByRole('dialog', { name: 'Sync with Google Drive' }).getByRole('button', { name: 'Sync now' }).click()
  await expect(a.getByRole('dialog', { name: 'Sync with Google Drive' }).getByRole('definition').first()).toHaveText(/Synced/)
  await b.reload()
  ...
  await panel(b).getByRole('button', { name: 'Sync settings' }).click()
  const conflictDialog = b.getByRole('dialog', { name: 'Sync with Google Drive' })
